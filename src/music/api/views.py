...
class ArtistViewSet(ListModelMixin, GenericViewSet):
    queryset = serializers.ArtistSerializer.optimize_queryset(models.Artist.objects).all()
...
class AlbumViewSet(ListModelMixin, GenericViewSet):
    ...
    def get_queryset(self):
        queryset = serializers.AlbumSerializer.optimize_queryset(models.Album.objects).all()
